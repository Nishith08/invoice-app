...
function InvoiceHistoryPage({role, invoiceId, onBack }) {
  ...
  useEffect(() => {
    fetch(`http://192.168.2.52:8000/api/logs/invoice/${invoiceId}`, {
  ...
  const handlePrintEntirePage = () => {
    ...
    const fullHTML = `
      ...
      <head>
        ...
        <title>Invoice History - Print</title>
  ...
  return (
    ...
    <div id="print-area" className="container" style={{ position: "relative", padding: 16, overflowY: "auto", height: "100%" }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {role !== 'admin' && (
            <button
              onClick={handlePrintEntirePage}
              aria-label="Print"
              title="Print Entire Page"
              style={{
                background: 'white',
                color: '#085EE3',
                padding: '6px 12px',
      ...
      {invoice?.final_document && (
        <div style={{ margin: '14px 0' }}>
          <a
            href={`http://192.168.2.52:8000/storage/${invoice.final_document}`}
            target="_blank"
